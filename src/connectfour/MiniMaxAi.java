...
public class MiniMaxAi {
	...
	// Variable that holds the maximum depth the MiniMax algorithm will reach for
	// this player
	...
	// The max and min functions are called interchangeably, one after another until
	...
	public Move maxAlphaBeta(Board board, int depth, double a, double b) {
		 ...
		 * reached, then a heuristic is calculated on the state and the move returned.
		 */
		if ((board.checkForGameOver()) || (depth == maxDepth)) {
			Move lastMove = new Move(board.getLastMove().getRow(), board.getLastMove().getColumn(), board.evaluate());
			return lastMove;
