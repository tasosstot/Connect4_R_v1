...
public class Board {
	...
	/*
	 * Generates the children of the state. The max number of the children is
	 * "numOfColumns".
	 */
	 ...
	 * Player 2. +10 for each 3 pieces in a row by Player 1, -10 for each 3 pieces
	 * in a row by Player 2. .. +10^i for each (i+2) pieces in a row by Player 1,
	 * -10^i for each (i+2) pieces in a row by Player 2. +10^(inARow-2) if "inARow"
	 * pieces in a row by Player 1 exist, -10^(inARow-2) if "inARow" pieces in a row
	 * by Player 2 exist.
	 */
	public int evaluate() {
		int player1Score = 0;
	...
	/*
	 * Terminal win check. It checks whether somebody has won the game.
	 */
	...
	public int countNInARow(int N, int player) {
		...
		// Check for "inARow" consecutive checkers of the same player or empty tiles in
		// a row, horizontally.
		for (int i = 0; i < numOfRows; i++) {
			for (int j = 0; j < numOfColumns; j++) {
				if (canMove(i, j + inARow - 1)) {
					...
					// Check for "inARow - N" consecutive checkers of the same player or empty tiles
		...
		for (int i = 0; i < numOfRows; i++) {
			for (int j = 0; j < numOfColumns; j++) {
				if (canMove(i + inARow - 1, j + inARow - 1)) {
					...
					// Check for "inARow - N" consecutive checkers of the same player or empty tiles
					// in a row, in descending diagonal.
					if (k == inARow) {
						while (k < inARow
		...
		for (int i = 0; i < numOfRows; i++) {
			for (int j = 0; j < numOfColumns; j++) {
				if (canMove(i - inARow + 1, j + inARow - 1)) {
					...
					// Check for "inARow - N" consecutive checkers of the same player or empty tiles
					// in a row, in ascending diagonal.
					if (k == inARow) {
						while (k < inARow
								&& (gameBoard[i - k][j + k] == player || gameBoard[i - k][j + k] == Constants.EMPTY)) {
							k++;
